...
void loop() 
{
  ...
  // freq range 20 to ~8.1K (1024 * 8 + 20)
  int freq = 20 + (analogRead(A2) * 8);
  Serial.print("Audio Freq: "); Serial.println(freq);

  // audo amplitude 0.0 to 1.0
  float amp = analogRead(A3)/1023.0;
  Serial.print("Audo Amp  : "); Serial.println(amp,2);
  
  waveform1.frequency(freq); // was (440);
  waveform1.amplitude(amp);  // was (0.9)
  wait(250);
