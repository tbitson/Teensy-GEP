    ...
    if (c > 32 && c < 127)
    {
      ...
      switch (c)
      {
        ...
        case '?':
          ...
          Serial.println("t : threshold increase");
          Serial.println("T : threshold decrease");
      ...
      //audioShield.autoVolumeControl(gain, response, limit, threshold, attack, decay);
      //compression(float threshold = -40.0f, float attack = MIN_T, float release = 0.5f, float ratio = 35.0f, float kneeWidth = 6.0f)
      audio.compression(threshold, attack, decay);
    }
    else
      delay(200);
  }
